...
class ProgramRoleController extends Controller
{
    ...
    public function store(Request $request, $programId)
    {
        ...
        $role = ProgramRole::create([
            ...
            'status' => 'active',
            'services' => $request->service_ids ?? [], // Store service permission strings
        ]);
    ...
    public function update(Request $request, $programId, $roleId)
    {
        ...
        // Update services (permission strings)
        if ($request->has('service_ids')) {
            $updateData['services'] = $request->service_ids;
        }

        $role->update($updateData);

        // Update event assignments (these are actual Activity UUIDs from database)
        if ($request->has('event_ids')) {
            $role->events()->sync($request->event_ids);
        }

        $role->load(['events', 'program']);
