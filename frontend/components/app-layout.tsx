...
import { getNavigationItems, UserRole } from "@/lib/permissions";
...
const searchablePages: { [key: string]: { placeholder: string; items: any[] } } = {
  ...
  '/programs': { placeholder: 'Search programs...', items: [] },
  '/participants': { placeholder: 'Search participants...', items: [] },
  '/questionnaires': { placeholder: 'Search questionnaires...', items: [] },
  '/activities': { placeholder: 'Search events...', items: [] },
  '/reports': { placeholder: 'Search reports...', items: [] },
  '/analytics': { placeholder: 'Search activities...', items: [] },
  '/dashboard': { placeholder: 'Search...', items: [] },
  '/super-admin': { placeholder: 'Search...', items: [] },
};
...
export default function AppLayout({ children }: AppLayoutProps) {
  ...
  useEffect(() => {
    if (currentUser) {
      ...
      const navItems = getNavigationItems(currentUser.role as UserRole);
        ...
        <header className="h-16 bg-white border-b border-gray-200 sticky top-0 z-30">
          <div className="h-full px-4 lg:px-6 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              {/* Search */}
              <GlobalSearchBar
                placeholder={currentPageSearch.placeholder}
              ...
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-3 px-3 py-2 hover:bg-gray-50 rounded-xl transition-all duration-200 border-2 border-transparent hover:border-qsights-blue/20"
                >
                  {isLoading ? (
                    <div className="w-10 h-10 bg-gray-300 rounded-full animate-pulse"></div>
                  ) : (
                    <div className="w-10 h-10 bg-gradient-to-br from-qsights-blue to-blue-600 rounded-full flex items-center justify-center text-white text-sm font-bold shadow-sm">
                  ...
                  <div className="hidden md:block text-left max-w-[180px]">
                    <p className="text-sm font-semibold text-gray-900 truncate">{currentUser?.name || "Loading..."}</p>
                    <p className="text-xs text-gray-500 lowercase truncate">{currentUser?.role?.replace('_', '-') || "admin"}</p>
                  </div>
                    ...
                    <div className="bg-gradient-to-br from-qsights-blue to-blue-600 px-5 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-blue-100 text-xs lowercase truncate">{currentUser?.role?.replace('_', '-') || "admin"}</p>
                        </div>
                      </div>
